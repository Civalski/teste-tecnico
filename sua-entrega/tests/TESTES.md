...
- `test_repeated_description_variant_is_reported_once_per_cd`: garante que a mesma variante divergente repetida em vários lotes gere somente uma anomalia por CD. Evita inflar o relatório por quantidade de lotes.
- `test_description_sales_and_coverage`: verifica descrição canônica, sinalização da variante divergente, soma dos saldos, vendas sem duplicação por lote e cálculo da cobertura. Protege os principais números do consolidado sem esconder diferenças cadastrais.
...
- `test_low_coverage_uses_exact_ratio_before_rounding`: valida o limite inclusivo de `0,25` mês e garante que a decisão use a razão exata, não o valor arredondado exibido.
- `test_zero_available_with_sales_is_reported_only_for_exact_zero`: sinaliza saldo disponível exatamente zerado com vendas, sem duplicar o tratamento específico de saldo negativo nem classificar saldo e vendas zerados.
- `test_expired_stock_with_sales_is_reported_once_per_product_and_cd`: gera um único alerta investigativo por produto/CD, sem atribuir as vendas a um lote vencido específico.
...
- `test_alternative_date_format_identifies_each_affected_record`: confirma que cada data em `AAAA-MM-DD` gera uma anomalia rastreável por código e lote.
- `test_short_code_anomaly_uses_normalized_code_and_preserves_original`: garante sete dígitos no relatório e mantém o código curto recebido dentro da ação sugerida.
...
- `test_real_files_generate_expected_outputs`: executa o ETL sobre os quatro arquivos de `dados/` e confere quantidades, totais, validade, saldo negativo, incompletude e descrições divergentes em produtos conhecidos. É a proteção final contra regressões que passem nos testes isolados, mas alterem a entrega real.
